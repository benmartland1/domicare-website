...
function band_color(band: Report["visibility_band"]): string {
  switch (band) {
    ...
    case "Visible":
      return "text-[color:var(--color-sky)]";
  }
}
...
function highlight(text: string, company: string, url: string): string {
  ...
  for (const n of needles) {
    ...
    out = out.replace(re, '<mark class="bg-[color:var(--color-sky)]/30 text-[color:var(--color-navy)] px-0.5 rounded">$1</mark>');
  }
  return out;
}
  ...
  if (!authed) {
    return (
      <form onSubmit={handleAuth} className="card mx-auto mt-8 max-w-md p-8">
        <label htmlFor="token" className="block text-[11px] font-medium uppercase tracking-[0.24em] text-[color:var(--color-slate-2)]">
          Access password
        </label>
        <input
          id="token"
          ...
          className="mt-3 w-full rounded-md border border-[color:var(--color-line-strong)] bg-[color:var(--color-surface)] px-4 py-3 text-base text-[color:var(--color-navy)] focus:border-[color:var(--color-sky)]/60 focus:outline-none"
  ...
  return (
    <div className="space-y-12">
      ...
      <form onSubmit={handleRun} className="card grid gap-5 p-8 sm:grid-cols-2">
        <div>
          <label htmlFor="company" className="block text-[11px] font-medium uppercase tracking-[0.24em] text-[color:var(--color-slate-2)]">
            Brand name
          </label>
          <input
            id="company"
            ...
            className="mt-2 w-full rounded-md border border-[color:var(--color-line-strong)] bg-[color:var(--color-surface)] px-4 py-3 text-base text-[color:var(--color-navy)] focus:border-[color:var(--color-sky)]/60 focus:outline-none"
            placeholder="e.g. Taxd"
          />
        </div>
        <div>
          <label htmlFor="url" className="block text-[11px] font-medium uppercase tracking-[0.24em] text-[color:var(--color-slate-2)]">
            Website
          </label>
          <input
            id="url"
        ...
        <div>
          <label htmlFor="industry" className="block text-[11px] font-medium uppercase tracking-[0.24em] text-[color:var(--color-slate-2)]">
            Industry
          </label>
          <select
            id="industry"
        ...
        <div>
          <label htmlFor="location" className="block text-[11px] font-medium uppercase tracking-[0.24em] text-[color:var(--color-slate-2)]">
            Primary market
          </label>
          <select
            id="location"
        ...
        <div className="sm:col-span-2">
          <label htmlFor="description" className="block text-[11px] font-medium uppercase tracking-[0.24em] text-[color:var(--color-slate-2)]">
            What they actually do <span className="lowercase tracking-normal text-[color:var(--color-slate)]/40">(optional but improves accuracy)</span>
          </label>
          <textarea
            id="description"
            value={description}
          ...
          />
          <p className="mt-1 text-[11px] text-[color:var(--color-slate-2)]">
      ...
      {error && (
        <div className="card border-red-500/40 bg-red-500/5 p-6 text-red-300">
          ...
          {diagnosis && (
            <pre className="mt-4 overflow-auto rounded-md border border-[color:var(--color-line)] bg-[color:var(--color-canvas-2)] p-4 text-xs text-[color:var(--color-slate)]">
              {diagnosis}
            </pre>
          )}
        </div>
      ...
      {loading && (
        <div className="card flex items-center gap-4 p-6 text-[color:var(--color-slate)]">
          <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-[color:var(--color-sky)]" />
  ...
  return (
    <div className="relative inline-flex shrink-0 items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <div className="mt-2 text-[10px] uppercase tracking-[0.28em] text-[color:var(--color-slate-2)]">
          / 100
        </div>
...
function StackedBar({ recommended, mentionedOnly, missing }: { recommended: number; mentionedOnly: number; missing: number }) {
  ...
  return (
    <div>
      <div className="flex h-3 w-full overflow-hidden rounded-full bg-[color:var(--color-surface)]">
        {recommended > 0 && (
          <div style={{ width: `${rec}%`, background: "#14c2b0" }} title={`Recommended: ${recommended}`} />
        )}
        {mentionedOnly > 0 && (
      ...
      <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-xs text-[color:var(--color-slate)]">
        <span className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-sm" style={{ background: "#14c2b0" }} />
...
function statusPill(p: Analysis) {
  ...
  if (p.target_recommended) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-[color:var(--color-sky)]/15 px-3 py-1 text-xs font-medium text-[color:var(--color-sky)]">
        <span className="h-1.5 w-1.5 rounded-full bg-[color:var(--color-sky)]" />
        Recommended
      </span>
    );
  }
...
function Report({ report }: { report: Report }) {
  ...
  return (
    <div className="space-y-10 print:space-y-6">
      ...
      <div className="card relative overflow-hidden p-8 sm:p-12">
        ...
        <div className="relative flex flex-col items-center gap-10 lg:flex-row lg:items-center lg:gap-14">
          ...
          <div className="flex-1 text-center lg:text-left">
            <div className="text-[10px] uppercase tracking-[0.28em] text-[color:var(--color-sky)]">
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-2 text-xs lg:justify-start">
              <span className="rounded-full border border-[color:var(--color-line)] bg-[color:var(--color-surface)] px-3 py-1 text-[color:var(--color-slate)]">
                {report.industry}
              </span>
              <span className="rounded-full border border-[color:var(--color-line)] bg-[color:var(--color-surface)] px-3 py-1 text-[color:var(--color-slate)]">
                {report.location}
              ...
              <a
                ...
                target="_blank"
                rel="noopener"
                className="rounded-full border border-[color:var(--color-sky)]/30 bg-[color:var(--color-sky)]/5 px-3 py-1 text-[color:var(--color-sky)] hover:bg-[color:var(--color-sky)]/10"
              >
                {report.url.replace(/^https?:\/\//, "")} ↗
              </a>
            </div>
            <p className="mt-4 text-xs text-[color:var(--color-slate-2)]">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-3">
          {[
            { label: "Recommended", value: report.recommended, total: report.total, accent: "#14c2b0" },
            ...
            <div key={kpi.label} className="relative overflow-hidden rounded-2xl border border-[color:var(--color-line)] bg-[color:var(--color-surface)] p-6">
              <span aria-hidden className="absolute left-0 top-0 h-full w-[2px]" style={{ background: kpi.accent }} />
              <div className="text-[10px] uppercase tracking-[0.22em] text-[color:var(--color-slate-2)]">
                {kpi.label}
              </div>
              <div className="display mt-3 text-4xl text-[color:var(--color-navy)]">
                {kpi.value}
                {kpi.total !== null && (
                  <span className="ml-1 text-xl text-[color:var(--color-slate)]/40">/ {kpi.total}</span>
                )}
              </div>
            </div>
          ))}
        ...
        {/* ============ STACKED BAR ============ */}
        <div className="mt-8">
          <div className="mb-3 text-[10px] uppercase tracking-[0.22em] text-[color:var(--color-slate-2)]">
            Prompt outcomes
          </div>
      ...
      {report.top_competitors.length > 0 && (
        <div className="card p-8 sm:p-10">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <div>
              <div className="text-[10px] uppercase tracking-[0.22em] text-[color:var(--color-sky)]">
                Top rivals
              </div>
              <h3 className="display mt-2 text-2xl sm:text-3xl">
                Brands ChatGPT recommended <span className="text-[color:var(--color-slate)]/50">instead</span>
              </h3>
            </div>
            <p className="text-xs text-[color:var(--color-slate-2)]">
            ...
            {report.top_competitors.map((c, i) => {
              ...
              return (
                <li key={c.name} className="grid grid-cols-[28px_1fr_60px] items-center gap-4">
                  <span className="text-sm font-medium text-[color:var(--color-slate-2)]">{i + 1}.</span>
                  <div className="flex items-center gap-3">
                    <span className="min-w-[160px] text-sm font-medium text-[color:var(--color-navy)]">
                      {c.name}
                    </span>
                  ...
                  <span className="text-right text-sm tabular-nums text-[color:var(--color-slate)]">
      ...
      <div>
        ...
        <div className="space-y-3">
          {report.prompts.map((p, i) => (
            <div key={i} className="card group p-6 sm:p-8 transition-colors hover:border-[color:var(--color-line-strong)]">
              <div className="flex flex-wrap items-start gap-4">
                <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-[color:var(--color-line)] bg-[color:var(--color-surface)] text-sm font-medium text-[color:var(--color-slate-2)]">
                  {i + 1}
                </div>
              ...
              {p.answer_summary && (
                <p className="ml-14 mt-4 border-l-2 border-[color:var(--color-sky)]/30 pl-4 text-sm leading-relaxed text-[color:var(--color-slate)]">
                  {p.answer_summary}
                </p>
              )}
                ...
                <div className="ml-14 mt-5">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {p.competitors.map((c, j) => (
                      <span
                        key={j}
                        className="rounded-md border border-[color:var(--color-line)] bg-[color:var(--color-surface)] px-2.5 py-1 text-xs text-[color:var(--color-slate)]"
                      >
                        {c}
                      </span>
                    ))}
              ...
              {p.response && (
                <details className="ml-14 mt-5 group/details">
                  <summary className="cursor-pointer text-[10px] uppercase tracking-[0.22em] text-[color:var(--color-slate-2)] transition-colors hover:text-[color:var(--color-sky)] group-open/details:text-[color:var(--color-sky)]">
                    Read full ChatGPT response ↓
                  </summary>
                  <div
                    className="prose prose-invert mt-4 max-w-none whitespace-pre-wrap rounded-lg border border-[color:var(--color-line)] bg-[color:var(--color-surface)] p-5 text-sm leading-relaxed text-[color:var(--color-slate)]"
      ...
      {/* ============ METHODOLOGY FOOTER ============ */}
      <div className="rounded-2xl border border-[color:var(--color-line)] bg-[color:var(--color-surface)] p-6 text-xs leading-relaxed text-[color:var(--color-slate-2)]">
        <div className="text-[10px] uppercase tracking-[0.22em] text-[color:var(--color-slate-2)]">
          Methodology
        </div>
        <p className="mt-2">
        ...
        <p className="mt-2">
          Snapshot generated by <span className="text-[color:var(--color-sky)]">DomiCare</span> on {generated}.
        </p>
      </div>
