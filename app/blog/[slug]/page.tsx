...
export default async function BlogPostPage({ params }: Props) {
  const { slug } = await params;
  const post = getPostBySlug(slug);
  // Drafts and posts awaiting review sit in the same directory as live posts.
  // They are only ever reachable through the CMS preview route.
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl px-6 pb-24 pt-16 lg:pt-24">
          <ScrollReveal>
            <Link
              href="/blog"
              className="text-xs uppercase tracking-[0.2em] text-[color:var(--color-sky)]"
            >
              ← Back to blog
            </Link>
          </ScrollReveal>
          ...
          <ScrollReveal delay={80}>
            <div className="mt-8 flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.2em] text-[color:var(--color-slate)]">
              {post.tags.map((t) => (
                <span key={t} className="rounded-full border border-[color:var(--color-line)] px-3 py-1">
                  {t}
                </span>
              ))}
            </div>
          ...
          <ScrollReveal delay={200}>
            <div className="mt-8 flex items-center gap-3 text-sm text-[color:var(--color-slate)]">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, var(--color-paper) 0%, var(--color-paper) 50%, var(--color-canvas) 50%, var(--color-canvas) 100%)",
        }}
      />
            ...
            {related.map((p, i) => (
              <ScrollReveal key={p.slug} delay={i * 80}>
                <Link href={`/blog/${p.slug}`} className="card block h-full p-8">
                  <div className="text-xs uppercase tracking-[0.2em] text-[color:var(--color-sky)]">
                    {p.tags[0] ?? "Article"}
                  </div>
                  <h3 className="display mt-4 text-xl leading-tight text-balance sm:text-2xl">
                    {p.title}
                  </h3>
                  <p className="mt-3 text-sm text-[color:var(--color-slate)]">{p.excerpt}</p>
                </Link>
              </ScrollReveal>
            ))}
          </div>
