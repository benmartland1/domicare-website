...
/**
 * Editorial state. Posts written before the CMS existed have no `status` in
 * their frontmatter and are treated as published, so nothing already live
 * disappears.
 */
export type PostStatus = "draft" | "review" | "published";

export const POST_STATUSES: PostStatus[] = ["draft", "review", "published"];

function normaliseStatus(value: unknown): PostStatus {
  return POST_STATUSES.includes(value as PostStatus) ? (value as PostStatus) : "published";
}
...
export function getPostBySlug(slug: string): Post | null {
  ...
  return {
    ...
    date: fm.date,
    excerpt: fm.excerpt ?? "",
    author: fm.author ?? "DomiCare Team",
    tags: fm.tags ?? [],
    status: normaliseStatus((data as Record<string, unknown>).status),
...
/**
 * Published posts only. Drafts and posts awaiting review live in the same
 * directory but must never reach the blog listing, sitemap or llms.txt.
 */
...
/** Slugs safe to pre-render publicly. Used by generateStaticParams. */
